...
// LM Studio config shape
// Path: ~/.cache/lm-studio/mcp-config.json

type LMStudioConfig struct {
	MCPServers map[string]LMStudioServer `json:"mcpServers"`
}

type LMStudioServer struct {
...
func (Adapter) Name() string { return "lmstudio" }
...
func (Adapter) Path() (string, error) {
	...
	return filepath.Join(h, ".cache", "lm-studio", "mcp-config.json"), nil
}
...
func (a Adapter) Load() (*LMStudioConfig, error) {
	p, err := a.Path()
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		if os.IsNotExist(err) {
			return &LMStudioConfig{MCPServers: map[string]LMStudioServer{}}, nil
		}
		return nil, err
	}
	var c LMStudioConfig
	if err := json.Unmarshal(b, &c); err != nil {
		return nil, err
	}
	if c.MCPServers == nil {
		c.MCPServers = map[string]LMStudioServer{}
	}
	return &c, nil
}
...
// Apply merges canonical servers into LM Studio config, preserving unmanaged entries.
func (a Adapter) Apply(canon *config.Canonical) (string, error) {
	...
	newCfg := LMStudioConfig{MCPServers: map[string]LMStudioServer{}}
	...
	for name, s := range enabled {
		newCfg.MCPServers[name] = LMStudioServer{
			Command: s.Command,
			Args:    s.Args,
			Env:     s.Env,
		}
